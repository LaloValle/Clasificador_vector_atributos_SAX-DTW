"""
    ...
    but changes in the recognition phase where de 1-NN classifier uses the DTW-SAX distance to measure the similarity between instances.
...
def tests():
    ...
    # DTW algorithm
    path = dtw.warping_path([int(digit) for digit in sax_time_series_3],[int(digit) for digit in sax_time_series_2])
    # Visualisation of the optimal correspondence between sines
    figure,ax = dtwvis.plot_warping([int(digit) for digit in sax_time_series_3], [int(digit) for digit in sax_time_series_2], path)
